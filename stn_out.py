...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("checkpoints/epoch_1500.pth", map_location=device))
    ...
    for i, (original, result) in enumerate(zip(images, results)):
        # print(torch.max(original), torch.max(result))
        print(original.dtype, result.dtype)
        to_pil_image(original).save(f"results/original_{i}.jpeg")
        to_pil_image(result).save(f"results/result_{i}.jpeg")

    original_grid_img.save("results/grid_original.jpeg")
    results_grid_img.save("results/grid_results.jpeg")
    ...
    ).save("results/grid_combined.jpeg")
